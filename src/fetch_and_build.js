...
(async()=>{
  ...
  for(const f of feeds){
    try{
      ...
      for(const it of feed.items.slice(0,20)){
        ...
        // create summary placeholder; actual summary to be generated by GitHub Action using model
        newItems.push({
  ...
  for(let p=1;p<=totalPages;p++){
    ...
    const rows = pageItems.map(it=>`<article><h2><a href="${it.link}" target="_blank" rel="noopener">${it.title}</a></h2><p class="meta">${it.source} — ${new Date(it.pubDate).toLocaleString()}</p><p class="summary">${it.summary}</p></article>`).join('\n');
